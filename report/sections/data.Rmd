...
Before we get into the methods used and the analysis, let's do a preliminary analysis on the correlations between the quantitative variables.

```{r, out.width = "500px", echo=FALSE}
knitr::include_graphics("../images/correlation-matrix.png")
```

Based on the above correlation visualization, we can see that _Balance_, _Income_, _Limit_, and _Rating_ have quite strong correlations with each other. This might present a problem when we use these variables during linear regression. We can demonstrate the same relationships with the following scatterplot matrix as well.
